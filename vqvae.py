...
def cartesian_to_spherical(cartesian_coords):
    ...
    phi = torch.atan2(y, x)
    r = (r / 1.74 * (256-1)).long()
    theta = (theta / 3.15 * (180 - 1)).long()
    phi = ((phi + 3.15) / 6.3 * (360 - 1)).long()
...
def spherical_to_cartesian(spherical_coords):
    ...
    r = r.float() / 255 * 1.74
    theta = theta.float() / 179 * 3.15
    phi = phi.float() / 359 * 6.3 - 3.15
...
class Encoder(nn.Module):
    ...
    def forward(self, pc):
        ...
        idx = fps(pos, batch, ratio=self.ratio) # 0.0625  ################ FPS ##################


        row, col = knn(pos, pos[idx], self.k, batch, batch[idx])
        edge_index = torch.stack([col, row], dim=0)

        x = self.conv(pos, pos[idx], edge_index, self.basis)  ############ PointNet #############
...
class Autoencoder(nn.Module):
    ...
    def encode(self, x, bins=128):
        B, _, _ = x.shape

        z_e_x, centers = self.encoder(x) # B x T x C, B x T x 3  [-1, 1] -> [-.5, .5]
    ...
    def forward(self, x, points):
        ...
        z_e_x, z_q_x_st, centers_quantized, loss_vq, perplexity, encodings, _ = self.encode(x)
